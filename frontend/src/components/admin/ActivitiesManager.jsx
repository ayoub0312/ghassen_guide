...
const ActivitiesManager = () => {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this activity?')) {
            try {
                await fetch(getApiUrl(`activities/${id}`), {
                    method: 'DELETE'
                });
                fetchActivities();
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingActivity) {
                await fetch(getApiUrl(`activities/${editingActivity.id}`), {
                    method: 'PUT',
                    headers: {
                ...
                await fetch(getApiUrl('activities'), {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    ...
                    <div key={activity.id} className="activity-card">
                        <div className="activity-image-container">
                            <div className="activity-image-placeholder" style={{
                                ...
                                {activity.image ? (
                                    <img
                                        src={activity.image.startsWith('http') ? activity.image : `${BASE_URL}/uploads/${activity.image}`}
                                        alt={activity.name}
                                        ...
                                        <label htmlFor="image-upload" className="image-upload-label">
                                            {formData.image ? (
                                                <div className="image-preview">
                                                    <img
                                                        src={formData.image.startsWith('http') ? formData.image : `${BASE_URL}/uploads/${formData.image}`}
                                                        alt="Preview"
                                                        onError={(e) => {
